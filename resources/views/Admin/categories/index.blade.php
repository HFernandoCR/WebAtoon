<x-app-layout>
    ...
    <div style="display: flex; min-height: calc(100vh - 65px);">
        ...
        <div style="flex: 1; padding: 30px; background-color: #f3f4f6;">
            <div class="card" style="background: white; padding: 20px; border-radius: 10px; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                    <h3 style="font-size: 1.2rem; font-weight: bold; color: #2c3e50;">Listado de Categorías</h3>
                    <a href="{{ route('categories.create') }}" style="background-color: #3498db; color: white; padding: 8px 15px; border-radius: 5px; text-decoration: none;">Nueva Categoría</a>
                </div>
                        ...
                        @foreach($categories as $category)
                            <tr>
                                ...
                                <td style="padding: 12px; border-bottom: 1px solid #ddd;">
                                    <a href="{{ route('categories.edit', $category) }}" style="color: #f39c12; margin-right: 10px;">Editar</a>
                                    <form action="{{ route('categories.destroy', $category) }}" method="POST" style="display: inline-block;" onsubmit="return confirm('¿Estás seguro?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" style="background: none; border: none; color: #e74c3c; cursor: pointer;">Eliminar</button>
                                    </form>
                                </td>
                            </tr>
                        @endforeach
