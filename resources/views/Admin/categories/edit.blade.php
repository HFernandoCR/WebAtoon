    ...
    <div style="display: flex; min-height: calc(100vh - 65px);">
        ...
        <div style="flex: 1; padding: 30px; background-color: #f3f4f6;">
            <div class="card" style="background: white; padding: 20px; border-radius: 10px; box-shadow: 0 2px 5px rgba(0,0,0,0.05); max-width: 600px; margin: 0 auto;">
                <h3 style="font-size: 1.2rem; font-weight: bold; color: #2c3e50; margin-bottom: 20px;">Editar Categoría</h3>
                ...
                <form action="{{ route('categories.update', $category) }}" method="POST">
                    ...
                    <div style="margin-bottom: 15px;">
                        ...
                        <input type="text" name="code" value="{{ old('code', $category->code) }}" required style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px;">
                        @error('code') <small style="color: red;">{{ $message }}</small> @enderror
                    </div>

                    <div style="margin-bottom: 15px;">
                        <label style="display: block; margin-bottom: 5px; font-weight: bold;">Nombre</label>
                        <input type="text" name="name" value="{{ old('name', $category->name) }}" required style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px;">
                        @error('name') <small style="color: red;">{{ $message }}</small> @enderror
                    </div>

                    <div style="margin-bottom: 15px;">
                        <label style="display: block; margin-bottom: 5px; font-weight: bold;">Descripción</label>
                        <textarea name="description" rows="3" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px;">{{ old('description', $category->description) }}</textarea>
                    </div>
                    ...
                    <div style="display: flex; justify-content: flex-end; gap: 10px;">
                        ...
                        <button type="submit" style="padding: 10px 20px; background: #f39c12; color: white; border: none; border-radius: 5px; cursor: pointer;">Actualizar</button>
                    </div>
                </form>
            </div>
        </div>
