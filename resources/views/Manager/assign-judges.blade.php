<x-app-layout>
    ...
    <div style="display: flex; min-height: calc(100vh - 65px);">
        ...
        <div style="flex: 1; padding: 30px; background-color: #f3f4f6;">
            ...
            <a href="{{ route('manager.dashboard') }}" style="display: inline-block; margin-bottom: 20px; color: #7f8c8d; text-decoration: none;">
                ← Volver al Panel
            </a>
            ...
            <div style="display: grid; grid-template-columns: 1fr 2fr; gap: 30px;">
                ...
                <div style="background: white; padding: 20px; border-radius: 10px; height: fit-content;">
                    <h4 style="font-weight: bold; margin-bottom: 15px; border-bottom: 1px solid #eee; padding-bottom: 10px;">Agregar Juez</h4>
                    
                    <form action="{{ route('manager.projects.add_judge', $project) }}" method="POST">
                        @csrf
                        <div style="margin-bottom: 15px;">
                            <label style="display: block; margin-bottom: 5px; font-weight: 600;">Seleccionar Juez</label>
                            <select name="judge_id" required style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px;">
                            ...
                            @if($availableJudges->isEmpty())
                                <small style="color: #e67e22; display: block; margin-top: 5px;">No hay más jueces disponibles.</small>
                            @endif
                        </div>
                        <button type="submit" style="background: #3498db; color: white; width: 100%; padding: 10px; border: none; border-radius: 5px; font-weight: bold; cursor: pointer;">
                            + Asignar
                        </button>
                    </form>
                </div>
                ...
                <div style="background: white; padding: 20px; border-radius: 10px;">
                    <h4 style="font-weight: bold; margin-bottom: 15px; border-bottom: 1px solid #eee; padding-bottom: 10px;">Jueces Asignados ({{ $assignedJudges->count() }})</h4>
                                ...
                                <tr style="border-bottom: 1px solid #eee;">
                                    ...
                                    <td style="padding: 10px; text-align: center;">
                                        @if($judge->pivot->score)
                                            <span style="color: #2ecc71; font-weight: bold;">Calificado ({{ $judge->pivot->score }})</span>
                                        @else
                                            <span style="color: #f1c40f; font-weight: bold;">Pendiente</span>
                                        @endif
                                    </td>
                                    <td style="padding: 10px; text-align: right;">
                                        <form action="{{ route('manager.projects.remove_judge', ['project' => $project->id, 'judgeId' => $judge->id]) }}" method="POST" onsubmit="return confirm('¿Quitar a este juez?');">
                                            @csrf @method('DELETE')
                                            <button style="color: #e74c3c; background: none; border: none; font-weight: bold; cursor: pointer;">Quitar</button>
                                        </form>
                                    </td>
                                </tr>
                                @endforeach
