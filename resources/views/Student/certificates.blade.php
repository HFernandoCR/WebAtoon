    ...
    <div class="flex-container" style="display: flex; min-height: calc(100vh - 65px);">
        ...
        <div class="main-content" style="flex: 1; padding: 30px; background-color: #f3f4f6;">
            ...
            @if($project && $project->status === 'approved')
                <div
                    style="background: white; padding: 30px; border-radius: 10px; display: flex; align-items: center; justify-content: space-between; border-left: 5px solid #2ecc71; flex-wrap: wrap; gap: 20px;">
                    <div>
                        <h3 style="font-weight: bold; font-size: 1.2rem;">Constancia de Participación</h3>
                        <p style="color: #666;">Evento: {{ $project->event->name }}</p>
                    </div>
                    <a href="{{ route('certificates.download') }}"
            ...
            @else
                <div style="background: white; padding: 40px; text-align: center; border-radius: 10px;">
                    ...
                    <p style="color: #999;">Las constancias se generan cuando el evento finaliza o tu proyecto es aprobado.
                    </p>
                </div>
            @endif
